...
\
  ...
  bit_len = cfg_all_table[codingset].lvl0_len - 1; table_ofst = !(codingset & 1) ? 0 : 0x400; \
\
  vlc_en = 1; cfg_mode = 1;\
...
#if 0
static av_always_inline int get_vlc2_tbl_hw(int codingset) {
  int hw_rslt;
  unsigned int vlc_len, vlc_mode, cfg_mode, vlc_en, ctrl_val;
  unsigned int tbl_mode, bit_len, table_ofst, cfg_val;

  int tbl_sel = codingset;
  tbl_mode = cfg_all_table[tbl_sel].table_mode;
  bit_len = cfg_all_table[tbl_sel].lvl0_len - 1; table_ofst = (codingset == vc1_hw_codingset1) ? 0 : 0x400;
