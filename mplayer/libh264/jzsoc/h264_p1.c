...
#include "h264_p1_idct.c"
#include "h264_p1_intra.c"


#include "h264_cabac_mc_hw.c"
...
__p1_main int main() {
  ...
  H264_Slice_GlbARGs *SLICE_T0=(int*)TCSM1_SLICE_BUF0;
  H264_Slice_GlbARGs *SLICE_T1=(int*)TCSM1_SLICE_BUF1;
  H264_MB_Ctrl_DecARGs *dmb_Z=TASK_BUF0;//last mb
  H264_MB_Ctrl_DecARGs *dmb_A=TASK_BUF1;//curr mb
  H264_MB_Ctrl_DecARGs *dmb_B=TASK_BUF2;//next mb

  uint8_t *motion_mc_dha1 = TCSM1_MC_DHA1;
  uint8_t *motion_mc_dsa1 = TCSM1_MC_DHA1 + 0x108;
  uint8_t *motion_mc_dha2 = TCSM1_MC_DHA2;
  uint8_t *motion_mc_dsa2 = TCSM1_MC_DHA2 + 0x108;
  unsigned int * idct_chain_ptr= IDCT_DES_CHAIN;
  unsigned int * idct_chain_ptr1= IDCT_DES_CHAIN1;
  ...
  uint8_t *dblk_upout_yaddr1 = TCSM1_DBLK_UPOUT_Y0;
  uint8_t *dblk_upout_yaddr2 = TCSM1_DBLK_UPOUT_Y1;
  
  uint8_t *dblk_upout_caddr1 = TCSM1_DBLK_UPOUT_U0;
  uint8_t *dblk_upout_caddr2 = TCSM1_DBLK_UPOUT_U1;
#endif

  uint8_t *edge_ybuf1 = TCSM1_EDGE_YBUF1;
  uint8_t *edge_cbuf1 = TCSM1_EDGE_CBUF1;
  uint8_t *edge_bak_ybuf1 = TCSM1_EDGE_BAK_YBUF1; 
  uint8_t *edge_bak_cbuf1 = TCSM1_EDGE_BAK_CBUF1; 
  ...
  *(volatile int*)DBLK_END_FLAG = DBLK_SYN_VALUE;
  ...
  mbnum=0;
  mbnum_wp=TCSM1_MBNUM_WP;
  mbnum_rp=TCSM1_MBNUM_RP;
  addr_rp=TCSM1_ADDR_RP;
  *mbnum_rp=0;
  *addr_rp=TCSM0_PADDR(TCSM0_TASK_FIFO);
  ...
  unsigned int error_add_cbp = 0 ,error_cbp = 0, last_cbp = 0;
  ...
  unsigned *intra_top_y=INTRAUP_Y+INTRAUP_STRD_Y-20;
  unsigned *intra_top_u=INTRAUP_U+INTRAUP_STRD_C-12;
  unsigned *intra_top_v=INTRAUP_V+INTRAUP_STRD_C-12;
  int *gp0_chain_ptr=DDMA_GP0_DES_CHAIN;
  int *gp1_chain_ptr = TCSM1_DDMA_GP1_DES_CHAIN1;
  int *gp1_chain_ptr2 = TCSM1_DDMA_GP1_DES_CHAIN2;
  ...
  *addr_rp+=dmb_A->next_mb_len;
  ...
  while(dmb_A->new_slice>=0){
    ...
    if(!IS_INTRA(dmb_A->mb_type)){
      motion_execute_cabac(SLICE_T0, dmb_A, motion_mc_dha1);
    }

    if (last_mc_run){
      while(*(volatile int *)motion_mc_dsa2 != (0x80000000 | 0xFFFF) ){};
      last_mc_run = 0;
    }
	...
	SET_REG1_WINFO(0,(SLICE_T0->use_weight == IS_WT1),SLICE_T0->use_weight,1,          \
		       SLICE_T0->luma_log2_weight_denom,5,0,0);
    ...
    if(!IS_INTRA(dmb_A->mb_type)){
      uint8_t *motion_douty = recon_ptr0;
      uint8_t *motion_doutc = recon_ptr0 + PREVIOUS_OFFSET_U;
      uint8_t *motion_dsa = motion_mc_dha1 + 0x108;      
    ...
    error_add_cbp = hw_idct_cabac_get_coeff(dmb_A, XCH2_T0->add_error_buf,idct_chain_ptr);
     ...
     * add error
     *---------------------------------------------------------------------*/
    //while (*(volatile int*)IDCT_END_FLAG != IDCT_SYN_VALUE);
    POLL_IDCT_END();
    func_add_error(last_cbp,recon_ptr2,XCH2_T1->add_error_buf);
    /*---------------------------------------------------------------------
     * back up intra bottom
     *---------------------------------------------------------------------*/
    intra_top_y[1]=((unsigned*)(recon_ptr2+15*PREVIOUS_LUMA_STRIDE))[0];
    intra_top_y[2]=((unsigned*)(recon_ptr2+15*PREVIOUS_LUMA_STRIDE))[1];
    intra_top_y[3]=((unsigned*)(recon_ptr2+15*PREVIOUS_LUMA_STRIDE))[2];
    intra_top_u[1]=((unsigned*)(recon_ptr2+PREVIOUS_OFFSET_U+7*PREVIOUS_CHROMA_STRIDE))[0];
    intra_top_v[1]=((unsigned*)(recon_ptr2+PREVIOUS_OFFSET_V+7*PREVIOUS_CHROMA_STRIDE))[0];

    if(mb_x){
      intra_top_y+=4;
      intra_top_u+=2;
      intra_top_v+=2;
      intra_topleft_tmp[0]=((unsigned*)(recon_ptr2+15*PREVIOUS_LUMA_STRIDE))[3];
      intra_topleft_tmp[1]=((unsigned*)(recon_ptr2+PREVIOUS_OFFSET_U+7*PREVIOUS_CHROMA_STRIDE))[1];
      intra_topleft_tmp[2]=((unsigned*)(recon_ptr2+PREVIOUS_OFFSET_V+7*PREVIOUS_CHROMA_STRIDE))[1];
    }else{
      intra_top_y[4]=((unsigned*)(recon_ptr2+15*PREVIOUS_LUMA_STRIDE))[3];
      intra_top_u[2]=((unsigned*)(recon_ptr2+PREVIOUS_OFFSET_U+7*PREVIOUS_CHROMA_STRIDE))[1];
      intra_top_v[2]=((unsigned*)(recon_ptr2+PREVIOUS_OFFSET_V+7*PREVIOUS_CHROMA_STRIDE))[1];
      intra_top_y=INTRAUP_Y;
      intra_top_u=INTRAUP_U;
      intra_top_v=INTRAUP_V;
    }

    hw_idct_cabac_config(dmb_A,intra_top_y,recon_ptr2,idct_chain_ptr);
    *(volatile int*)IDCT_END_FLAG=0;
    ...
    if((!IS_INTRA(dmb_Z->mb_type)) && mbnum){
      unsigned * Inter_Dec_base=((uint32_t)dmb_Z)+sizeof(struct H264_MB_Ctrl_DecARGs)+4;
      int is_bframe=SLICE_T0->slice_type-2;
      int mv_num=Inter_Dec_base[-1]<<is_bframe;
      int fix_len=is_bframe?21:11;
      unsigned *tmp_dblk_mv_ptr = XCH2_T1->dblk_mv_ptr - 4;
     ...
     * Synchronize with P0
     *---------------------------------------------------------------------*/
    //while(*mbnum_wp<=*mbnum_rp + 1);//wait until the next next mb is ready
    ...
    while(*mbnum_wp<=*mbnum_rp + 1);//wait until the next mb is ready
    ...
    *addr_rp+=dmb_B->next_mb_len;
	...
	Intra_pred_luma_16x16(INTRA_T->intra16x16_pred_mode,recon_ptr0,recon_ptr2+16,intra_top_y+1,dmb_A);

      Intra_pred_chroma(INTRA_T->chroma_pred_mode, recon_ptr0+PREVIOUS_OFFSET_U, 
			recon_ptr2+PREVIOUS_OFFSET_U+8, intra_top_u+1);
      Intra_pred_chroma(INTRA_T->chroma_pred_mode, recon_ptr0+PREVIOUS_OFFSET_V, 
  			recon_ptr2+PREVIOUS_OFFSET_V+8, intra_top_v+1);
    }
    ...
    //if (mbnum>1){
    // dblk should after back_up intra top and left
	...
	*(volatile int*)DBLK_END_FLAG=0;
    ...
    if (mbnum>1){
      ...
      for(i=0; i<2; i++){
        src1 = XCH2_T0->dblk_out_addr_c + 12*(8-tag2) + i*4;
	...
	int *gp1_chain_ptr=XCH2_T0->gp1_chain_ptr;
        gp1_chain_ptr[0]=TCSM1_PADDR(dblk_upout_yaddr1);
	gp1_chain_ptr[1]=(up_ydst + 16*12);
	...
	if ((mb_y_d3+1)==SLICE_T0->mb_height) {
	  gp1_chain_ptr[11]=GP_UNIT(GP_TAG_LK,1,16*(12+4));
	  gp1_chain_ptr[15]=GP_UNIT(GP_TAG_UL,1,16*(4+4));
	}
...
#endif    
    XCHG2(idct_chain_ptr,idct_chain_ptr1,xchg_tmp);
    XCHG2(XCH2_T0,XCH2_T1,xchg_tmp);
    XCHG2(motion_mc_dha1, motion_mc_dha2, xchg_tmp);
    XCHG2(motion_mc_dsa1, motion_mc_dsa2, xchg_tmp);
    last_cbp = error_add_cbp;
    mbnum++;
