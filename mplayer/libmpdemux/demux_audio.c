...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  while(n < 30000 && !s->eof) {
    int mp3_freq, mp3_chans, mp3_flen, mpa_layer, mpa_spf, mpa_br, mpassize;
	...
	step = 4;
      ...
      break;
                                ...
                                &mpa_spf, &mpa_layer, &mpa_br, &mpassize)) > 0) {
      mp3_found = add_mp3_hdr(&mp3_hdrs, st_pos, mp3_chans, mp3_freq,
                              mpa_spf, mpa_layer, mpa_br, mp3_flen, mpassize);
      if (mp3_found) {
        frmt = MP3;
        break;
      }
  ...
  case MP3: {
    ...
    int skip = 0;
    stream_seek(s,mpeghdr_pos);
    stream_read(s,vbrhdr,4);
    if(vbrhdr[0] == 'V' && vbrhdr[1] == 'B' && vbrhdr[2] == 'R' && vbrhdr[3] == 'I'){
      stream_skip(s,10);
      stream_read(s,framesnb_hdr,4);	//found frame number
      frames_nb = (framesnb_hdr[0]<<24) | (framesnb_hdr[1]<<16) | (framesnb_hdr[2]<<8) | (framesnb_hdr[3]);
      skip = 1;	//if found VBRI, skip Xing
    }	
    stream_seek(s,mpeghdr_pos+4+mp3_found->mpa_ssize);
    stream_read(s,vbrhdr,4);
    if(!s->eof && !skip)
      if(vbrhdr[0] == 'X' && vbrhdr[1] == 'i' && vbrhdr[2] == 'n' && vbrhdr[3] == 'g'){  // We found a XING header
    ...
    free(mp3_found);
    mp3_found = NULL;
    if(s->end_pos && (s->flags & MP_STREAM_SEEK) == MP_STREAM_SEEK && (s->type != STREAMTYPE_STREAM)) {
      char tag[4];
    ...
    if(frames_nb > 0){
      mp3_totaltime = frames_nb * sh_audio->audio.dwScale / sh_audio->audio.dwRate;
      sh_audio->wf->nAvgBytesPerSec = demuxer->movi_end / mp3_totaltime;
    }   
  } break;
  case WAV: {
