...
static demuxer_t* demux_open_lavf(demuxer_t *demuxer){
	...
	for(i=0; i < avfc->nb_streams; i++){
		...
		if(!strcmp(t->key, "width"))	//get from flv header
		    width = atoi(t->value);
		if(!strcmp(t->key, "height"))
		    height = atoi(t->value);
		if(!strcmp(t->key, "framerate"))
		    framerate = atoi(t->value);
	    }
	}
    }
    ...
    for(i=0; i < avfc->nb_streams; i++){
		...
		if(st->codec->codec_id == CODEC_ID_H264){
	    	...
	    	if(framerate <= 100 && framerate > 0){
				st->r_frame_rate.num = st->codec->time_base.den = framerate;
				st->r_frame_rate.den = st->codec->time_base.num = 1;
	    	}
		}
    }
