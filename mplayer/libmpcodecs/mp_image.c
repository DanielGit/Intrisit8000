...
#include "libvc1/jzsoc/t_vputlb.h"
...
void mp_image_alloc_planes(mp_image_t *mpi) {
  ...
  if(use_jz_buf)
...
#ifdef JZC_TLB_OPT
      size = (w+16)*(h) + (w+16) *(ch) + (mpi->width+16)*(mpi->height) + (mpi->width+16)*mpi->chroma_height+1024;
	...
	return -1;
      ...
      mpi->planes[0]=((int)(data + (w/16)*256*(EDGE_WIDTH/16) + 256*(EDGE_WIDTH/16) + 256)& ~(256-1));
#else
#if 1//def JZ_LINUX_OS
