...
#define IPU_DISPSEL       (1 << 14)
#define IPU_FILED_CONF_EN (1 << 15)
#define IPU_FILED_SELBOTT (1 << 16)
...
#define disable_ipu_field(IPU_V_BASE)  \
(REG32(IPU_V_BASE + REG_CTRL) &= ~IPU_DISPSEL)

#define enable_ipu_field_top(IPU_V_BASE)                           \
do {                                                               \
      volatile unsigned int ipureg = REG32(IPU_V_BASE + REG_CTRL); \
      volatile unsigned int masktop = (IPU_DISPSEL | IPU_FILED_CONF_EN); \
      REG32(IPU_V_BASE + REG_CTRL) = (ipureg | masktop);              \
    } while (0)

#define enable_ipu_field_bottom(IPU_V_BASE)                           \
do {                                                               \
      volatile unsigned int ipureg = REG32(IPU_V_BASE + REG_CTRL); \
      volatile unsigned int masktop = (IPU_DISPSEL | IPU_FILED_CONF_EN | IPU_FILED_SELBOTT); \
      REG32(IPU_V_BASE + REG_CTRL) = (ipureg | masktop);              \
    } while (0)
