...
struct IMEM_INFO
{
  ...
  unsigned int power;
  unsigned int vaddr;
} imem_info[4] = {{"/proc/jz/imem", 13/*32M*/, 0x0},
...
static void jz47_alloc_memdev ()
{
  ...
  unsigned char cmdbuf[128];
  unsigned int vaddr, paddr, vaddr_align;
  ...
  else
  {
    ...
    /* first, we need get the vaddr with align by MMAP_ALIGN. */
    vaddr = (unsigned int)mmap ((void *)imem_info[memdev_count].vaddr, 
                                (pages * PAGE_SIZE  + MMAP_ALIGN)* 2, /* since mips TLB table mmap is double pages.  */
                                (PROT_READ|PROT_WRITE), MAP_SHARED, mmapfd, paddr);
    vaddr_align = (vaddr + MMAP_ALIGN - 1) & ~(MMAP_ALIGN - 1);
    munmap((void *)vaddr, (pages * PAGE_SIZE  + MMAP_ALIGN)* 2);

    /* second, we need use vadd_align virtual addr to do mmap. */
    vaddr = (unsigned int)mmap ((void *)vaddr_align, 
