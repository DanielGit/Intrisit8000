...
static void do_apply_filter(APEContext * ctx, int version, APEFilter *f, int32_t *data, int count, int order, int fracbits)
{
    ...
    while (count--) {
#if 1 
	int16_t *coeffs_data, *delay_data, *adapt_data;
	int halforder;

	coeffs_data = f->coeffs;
	delay_data = f->delay - order;
	adapt_data = f->adaptcoeffs - order;
	halforder = order >> 1;

        S32LDD(xr1, coeffs_data, 0); 
        S16LDD(xr2, delay_data, 0x0, ptn0);
        S16LDD(xr2, delay_data, 0x2, ptn1);
        if(*data==0){
           D16MUL_WW(xr4, xr1, xr2, xr5);
           while(--halforder){
              S32LDI(xr1, coeffs_data, 0x4);
              S16LDI(xr2, delay_data, 0x4, ptn0);
              S16LDD(xr2, delay_data, 0x2, ptn1);
              D16MAC_AA_WW(xr4, xr1, xr2, xr5);
           }
        }else if(*data < 0){
           S16LDD(xr3, adapt_data, 0x0, ptn0);
           S16LDD(xr3, adapt_data, 0x2, ptn1);
           D16MUL_WW(xr4, xr1, xr2, xr5);
           Q16ADD_AS_WW(xr6, xr1, xr3, xr7);
           S32STD(xr6, coeffs_data, 0x0);
           while(--halforder){
              S32LDI(xr1, coeffs_data, 0x4);
              S16LDI(xr2, delay_data, 0x4, ptn0);
              S16LDD(xr2, delay_data, 0x2, ptn1);

              S16LDI(xr3, adapt_data, 0x4, ptn0);
              S16LDD(xr3, adapt_data, 0x2, ptn1);
              D16MAC_AA_WW(xr4, xr1, xr2, xr5);
              Q16ADD_AS_WW(xr6, xr1, xr3, xr7);
              S32STD(xr6, coeffs_data, 0x0);
