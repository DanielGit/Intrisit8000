...
void
av_fast_malloc (void *ptr, unsigned int *size, unsigned int min_size)
{
  void **p = ptr;
  if (min_size < *size)
    return;
...
void
register_avcodec (AVCodec * codec)
{
...
unsigned
avcodec_get_edge_width (void)
{
  return EDGE_WIDTH;
}

void
avcodec_set_dimensions (AVCodecContext * s, int width, int height)
{
  s->coded_width = width;
...
void
avcodec_align_dimensions2 (AVCodecContext * s, int *width, int *height,
                           int linesize_align[4])
{
  ...
  switch (s->pix_fmt)
    ...
    case PIX_FMT_YUVA420P:
      ...
      if (s->codec_id == CODEC_ID_MPEG2VIDEO || s->codec_id == CODEC_ID_MJPEG
          || s->codec_id == CODEC_ID_AMV || s->codec_id == CODEC_ID_THP)
  ...
  *height = FFALIGN (*height, h_align);
  ...
  linesize_align[0] =
    linesize_align[1] = linesize_align[2] = linesize_align[3] = STRIDE_ALIGN;
...
#if LIBAVCODEC_VERSION_MAJOR < 53
int
avcodec_check_dimensions (void *av_log_ctx, unsigned int w, unsigned int h)
{
...
#define ROA   256
  ...
  if (s->internal_buffer == NULL)
    {
      s->internal_buffer =
        av_mallocz ((INTERNAL_BUFFER_SIZE + 1) * sizeof (InternalBuffer));
    }
#if 0
  s->internal_buffer = av_fast_realloc (s->internal_buffer,
                                        &s->internal_buffer_size,
                                        sizeof (InternalBuffer) * FFMAX (99,
                                                                         s->
  ...
  if (buf->base[0]
      && (buf->width != w || buf->height != h || buf->pix_fmt != s->pix_fmt))
    {
      for (i = 0; i < 4; i++)
        {
          av_freep (&buf->base[i]);
  ...
  if (buf->base[0])
  ...
  else
    {
      ...
      tmpsize =
        av_image_fill_pointers (picture.data, s->pix_fmt, h, NULL,
                                picture.linesize);
      if (tmpsize < 0)
        return -1;
              ...
              if (!tlb_i)
                {
                  SET_VPU_TLB (0, 1, PSIZE_32M,
                               ...
                               get_phy_addr ((int) buf->base[i]) >> 22);
                  tlb_i++;
                  if (((GET_VPU_TLB (0) >> 25) & 0x3FF) !=
                      ((get_phy_addr ((int) buf->base[i] + total_size * 3) >>
                        25) & 0x3FF))
                    {
                      SET_VPU_TLB (tlb_i, 1, PSIZE_32M,
                                   get_phy_addr ((int) buf->base[i] +
                                                 total_size * 3) >> 22,
                                   get_phy_addr ((int) buf->base[i] +
                                                 total_size * 3) >> 22);
                      tlb_i++;
              ...
              else
                {
                  ...
                  for (tmp_i = 0; tmp_i < tlb_i; tmp_i++)
                    {
                      if (((GET_VPU_TLB (tmp_i) >> 25) & 0x3FF) ==
                          ((get_phy_addr ((int) buf->base[i]) >> 25) & 0x3FF))
                        {
                          start_hit++;
                          break;
                        }
          ...
          for (i = 0; i < 4 && size[i]; i++)
            {
              ...
              if ((s->flags & CODEC_FLAG_EMU_EDGE) || !size[2])
                ...
                buf->data[i] =
                  buf->base[i] +
                  FFALIGN ((buf->linesize[i] * EDGE_WIDTH >> v_shift) +
                           (EDGE_WIDTH >> h_shift), stride_align[i]);
            }
#endif
        }
      else
...
avcodec_default_release_buffer (AVCodecContext * s, AVFrame * pic)
{
  ...
  if (use_jz_buf)
    {
#ifdef USE_IPU_THROUGH_MODE
      if ((unsigned int) buf->data[0] != disp_buf0
          && (unsigned int) buf->data[0] != disp_buf1
          && (unsigned int) buf->data[0] != disp_buf2)
        {
          assert (i < s->internal_buffer_count);
          s->internal_buffer_count--;
          last =
            &((InternalBuffer *) s->internal_buffer)[s->
                                                     internal_buffer_count];
      ...
      s->internal_buffer_count--;
      last =
        &((InternalBuffer *) s->internal_buffer)[s->internal_buffer_count];
...
int
avcodec_default_reget_buffer (AVCodecContext * s, AVFrame * pic)
{
  AVFrame temp_pic;
  int i;
...
avcodec_default_execute2 (AVCodecContext * c,
                          int (*func) (AVCodecContext * c2, void *arg2,
                                       int jobnr, int threadnr), void *arg,
                          int *ret, int count)
{
  int i;
...
enum PixelFormat
avcodec_default_get_format (struct AVCodecContext *s,
...
#define SANE_NB_CHANNELS 128U
  if (((avctx->coded_width || avctx->coded_height)
       && av_image_check_size (avctx->coded_width, avctx->coded_height, 0,
                               avctx)) || avctx->channels > SANE_NB_CHANNELS)
    {
      ret = AVERROR (EINVAL);
      goto free_and_end;
    }
...
int attribute_align_arg
avcodec_encode_audio (AVCodecContext * avctx, uint8_t * buf, int buf_size,
                      const short *samples)
{
...
int attribute_align_arg
avcodec_encode_video (AVCodecContext * avctx, uint8_t * buf, int buf_size,
                      const AVFrame * pict)
{
...
int
avcodec_encode_subtitle (AVCodecContext * avctx, uint8_t * buf, int buf_size,
...
avcodec_decode_video (AVCodecContext * avctx, AVFrame * picture,
                      int *got_picture_ptr, const uint8_t * buf, int buf_size)
{
  AVPacket avpkt;
...
#if LIBAVCODEC_VERSION_MAJOR < 53
int attribute_align_arg
avcodec_decode_audio2 (AVCodecContext * avctx, int16_t * samples,
...
#if LIBAVCODEC_VERSION_MAJOR < 53
int
avcodec_decode_subtitle (AVCodecContext * avctx, AVSubtitle * sub,
                         int *got_sub_ptr, const uint8_t * buf, int buf_size)
{
  AVPacket avpkt;
  av_init_packet (&avpkt);
  avpkt.data = buf;
...
int
avcodec_decode_subtitle2 (AVCodecContext * avctx, AVSubtitle * sub,
...
static int
get_bit_rate (AVCodecContext * ctx)
{
  int bit_rate;
  int bits_per_sample;
  ...
  switch (ctx->codec_type)
    ...
    case AVMEDIA_TYPE_AUDIO:
      ...
      bit_rate =
        bits_per_sample ? ctx->sample_rate * ctx->channels *
  ...
  switch (enc->codec_type)
    ...
    case AVMEDIA_TYPE_VIDEO:
      snprintf (buf, buf_size,
                "Video: %s%s", codec_name, enc->mb_decision ? " (hq)" : "");
      if (enc->pix_fmt != PIX_FMT_NONE)
        {
          snprintf (buf + strlen (buf), buf_size - strlen (buf),
                    ", %s", avcodec_get_pix_fmt_name (enc->pix_fmt));
        }
      if (enc->width)
        {
          snprintf (buf + strlen (buf), buf_size - strlen (buf),
    ...
    case AVMEDIA_TYPE_AUDIO:
      ...
      avcodec_get_channel_layout_string (buf + strlen (buf),
                                         buf_size - strlen (buf),
...
unsigned
avcodec_version (void)
{
  return LIBAVCODEC_VERSION_INT;
}

const char *
avcodec_configuration (void)
{
  return FFMPEG_CONFIGURATION;
}

const char *
avcodec_license (void)
{
...
void
avcodec_default_free_buffers (AVCodecContext * s)
{
  int i, j;
...
int
av_get_bits_per_sample (enum CodecID codec_id)
{
  switch (codec_id)
    {
...
int
av_get_bits_per_sample_format (enum SampleFormat sample_fmt)
{
  switch (sample_fmt)
    {
    case SAMPLE_FMT_U8:
...
#if !HAVE_THREADS
int
avcodec_thread_init (AVCodecContext * s, int thread_count)
{
...
unsigned int
av_xiphlacing (unsigned char *s, unsigned int v)
{
  unsigned int n = 0;
...
int
av_parse_video_frame_size (int *width_ptr, int *height_ptr, const char *str)
{
  return av_parse_video_size (width_ptr, height_ptr, str);
}

int
av_parse_video_frame_rate (AVRational * frame_rate, const char *arg)
{
...
int
ff_match_2uint16 (const uint16_t (*tab)[2], int size, int a, int b)
{
  int i;
...
void
av_log_missing_feature (void *avc, const char *feature, int want_sample)
{
...
void
av_log_ask_for_sample (void *avc, const char *msg)
{
  if (msg)
...
AVHWAccel *
ff_find_hwaccel (enum CodecID codec_id, enum PixelFormat pix_fmt)
{
...
int
av_lockmgr_register (int (*cb) (void **mutex, enum AVLockOp op))
{
  if (ff_lockmgr_cb)
    {
...
ff_toupper4 (unsigned int x)
{
  return toupper (x & 0xFF)
    + (toupper ((x >> 8) & 0xFF) << 8)
    + (toupper ((x >> 16) & 0xFF) << 16) + (toupper ((x >> 24) & 0xFF) << 24);
}
