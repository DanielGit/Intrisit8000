	  ...
	  for(i=0; i<4; i++){
	    ...
	    const int v_shift= ((i%2)==0) ? 0 : v_chroma_shift;
	      ...
	      buf->linesize[i] = picture.linesize[i];  
	       ...
	       buf->linesize[i] = picture.linesize[0]- EDGE_WIDTH*2;          
    	       total_size = (picture.linesize[0] - EDGE_WIDTH*2)*havail + 256;
            }else{
               buf->linesize[i] = picture.linesize[0]- EDGE_WIDTH*2;          
	       total_size = (picture.linesize[0] - EDGE_WIDTH*2)*havail/2 + 128;
            }
	    ...
	    if((s->flags&CODEC_FLAG_EMU_EDGE) || !size[2])
	      buf->data[i] = buf->base[i];
              ...
              if (i < 2){
		buf->data[i] = buf->base[i] + FFALIGN((aline[i]*EDGE_WIDTH>>v_shift) + (ROA>>v_shift) + 
						    (EDGE_WIDTH>>h_shift), ROA);
              }else{
                buf->data[i] = buf->base[i];
              }
	    }
